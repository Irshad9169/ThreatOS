...
models/alert.py
────────────────
Alert — one detection hit produced by the rule engine.

Relationship to other models:
  rule_id  → detection_rules.id  (which rule fired)
  event_id → raw_events.id       (which event triggered it)

Status lifecycle:
  open → investigating → escalated → closed
                       ↘ false_positive

All numeric scoring components are stored individually so analysts
can see exactly why a score is what it is without re-computing it.
"""
...
from sqlalchemy import (
    CheckConstraint, DateTime, Float, Index,
...
class Alert(Base):
    ...
    rule_id: Mapped[str | None] = mapped_column(
        ...
        # Stored as string (not FK) so alerts survive rule deletion
    )
    ...
    # ── Scoring components (all stored individually) ──────────────────────────
    ...
    # ── Extra context for analyst triage ─────────────────────────────────────
    description: Mapped[str | None] = mapped_column(String(500), nullable=True)
    raw_match:   Mapped[dict | None] = mapped_column(JSONBCompat, nullable=True)
    # raw_match stores: rule_name, matched_fields, tags — for analyst context

    __table_args__ = (
        Index("ix_alerts_technique_id",  "technique_id"),
        Index("ix_alerts_tactic",        "tactic"),
