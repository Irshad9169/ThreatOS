...
# ── Auth: all /api/purple routes require an authenticated user (see
# threatos/api/routers/purple_router.py — Depends(get_current_user)).
...
def _rule(technique_id: str, value: str = "powershell") -> DetectionRule:
    # NOTE: DetectionRule.id is a String(36) column, not a native UUID type
    # — must pass a str.
    return DetectionRule(
        id=str(uuid.uuid4()), name=f"Rule {technique_id} {uuid.uuid4().hex[:6]}",
